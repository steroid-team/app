...
public class NoteSelectionActivityTest {
    ...
    public void openListWorks() {
        ...
        Espresso.onData(anything()).inAdapterView(withId(R.id.activity_noteselection_notelist)).atPosition(1).perform(click());

        Intents.intended(Matchers.allOf(IntentMatchers.hasComponent(NoteDisplayActivity.class.getName())));
        Intents.release();
    }
    ...
    public void openTodoListWorks() {
        ...
        onView(withId(R.id.activity_noteselection_button))
                .perform(click());

        Intents.intended(Matchers.allOf(IntentMatchers.hasComponent(ListSelectionActivity.class.getName())));
        Intents.release();
    }
